...
│   └── unit/                    # Unit testler (9 dosya, 138 test)
├── scripts/                     # CLI araçları
├── migrations/                  # Alembic migration'ları
├── docs/                        # Teknik dokümantasyon
├── bitirme veriler - öğrenci işleri/  # Kaynak dokümanlar
├── docker-compose.yml
...
### 4.5 GPU Desteği (Opsiyonel — Ollama için)

NVIDIA GPU'nuz varsa, `docker-compose.yml` dosyasında Ollama servisindeki GPU satırlarını açarak LLM performansını artırabilirsiniz:
...
- [x] Kaynak dokümanlar mevcut (`bitirme veriler - öğrenci işleri/` klasörü)
...
|-----------|------------|----------|
| `--source` | `bitirme veriler - öğrenci işleri` | Kaynak doküman klasörü |
| `--chunk-size` | `1024` | Chunk boyutu (karakter) |
| `--chunk-overlap` | `128` | Chunk örtüşmesi (karakter) |
| `--collection` | `student_affairs_docs` | ChromaDB koleksiyon adı |
...
==================================================
   Kaynak:        .../bitirme veriler - öğrenci işleri
   Chunk boyutu:  1024
   Chunk örtüşme: 128
...
> **Süre:** İlk indeksleme ~5-15 dakika sürer (embedding modeli yükleme + 5303 chunk vektörize etme). Sonraki çalıştırmalar sadece ~3-8 dakika (model cache'ten yüklenir).
...
| `test_rag_pipeline.py` | 29 | DocumentLoader, TextPreprocessor, TextChunker, hash ID üretimi |
| `test_query_preprocessor.py` | 22 | Bileşik kelime, sinonim, sorgu tipi |
| `test_embedder.py` | 14 | Model algılama, prefix, vektör üretimi |
| `test_reranker.py` | 11 | Cross-encoder sıralama |
| `test_retriever.py` | 25 | BM25, konu tespiti, source relevance, dedup, cache |

**Integration Testler (ChromaDB gerekli):**

| Test Dosyası | Test Sayısı | Kapsam |
|-------------|:-----------:|--------|
| `test_rag_retrieval.py` | 17 | ChromaDB bağlantı, koleksiyon, hibrit arama, cache, Precision@3 |
...
Bu komut:
- 20 test sorusunu HybridRetriever ile çalıştırır
...
4. python -m uvicorn ...             # API sunucusu
```
...
*Bu rehber, projenin mevcut durumunu (FAZ 0 + FAZ 1 tamamlanmış) yansıtmaktadır. FAZ 2 (LLM entegrasyonu, API gateway, Slack bot) tamamlandığında güncellenecektir.*
