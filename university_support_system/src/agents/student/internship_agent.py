...
class InternshipAgent(BaseSpecialistAgent):
    ...
    async def handle_department_task(self, task: Task) -> DepartmentResponse:
        ...
        if self._is_staj_summer_school_overlap_query(query_text):
            return DepartmentResponse(
                department=self.department,
                answer=(
                    "Genel kural olarak staj, ders veya yaz okulu ders ve sınav tarihleriyle çakışmayacak "
                    "şekilde ve ders devam zorunluluğunun bulunmadığı dönemlerde yapılmalıdır. Bu nedenle "
                    "staj tarihleriniz yaz okulu dersleriniz veya sınavlarınızla çakışıyorsa yaz okulundan "
                    "ders alamazsınız. Ancak staj ile yaz okulu tarihlerinin çakışmadığı durumlarda hem staj "
                    "yapıp hem de yaz okulundan ders alabilirsiniz."
                ),
                generation_mode="kural",
                success=True,
            )
    ...
    @classmethod
    def _is_staj_summer_school_overlap_query(cls, query_text: str) -> bool:
        lowered = normalize_text(query_text)
        return "staj" in lowered and any(
            marker in lowered for marker in ("yaz okulu", "yaz ogretimi", "yaz donemi")
        ) and any(
            marker in lowered for marker in ("cakis", "cakisma", "cakisirsa")
        )
