...
def _build_response_context_payload(response: DepartmentResponse, *, query: str) -> dict[str, object] | None:
    ...
    subtask = response.metadata.get("multi_intent_subtask") if isinstance(response.metadata, dict) else None
    if isinstance(subtask, dict):
        payload["multi_intent_subtask"] = {
            "subtask_id": subtask.get("subtask_id"),
            "title": subtask.get("user_facing_title"),
            "effective_question": subtask.get("effective_question"),
            "capability": subtask.get("capability"),
            "source_family": subtask.get("source_family"),
        }
    if evidence_items:
