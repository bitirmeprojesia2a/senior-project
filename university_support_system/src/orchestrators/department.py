...
def _contract_semantic_text(metadata: dict[str, Any] | None) -> str:
    ...
    subtask = _as_dict(metadata.get("multi_intent_subtask"))
    if subtask:
        resolved_q = metadata.get("resolved_query") or subtask.get("effective_question") or ""
        parts.append(str(resolved_q))
    else:
        planner = _as_dict(metadata.get("capability_planner"))
        action = _as_dict(planner.get("action"))
