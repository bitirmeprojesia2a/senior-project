...
def append_generation_summary(
    ...
    *,
    ...
    if agents_involved:
        lines.append(f"- Ajan akışı: {_format_agent_flow(routing_strategy, agents_involved, responses)}")
...
_AGENT_FLOW_LABELS = {
    ...
    "main_orchestrator": "Ana Orkestratör",
}

_DEPARTMENT_ORCHESTRATOR_LABELS = {
    Department.STUDENT_AFFAIRS: "Öğrenci İşleri Orkestratörü",
    Department.ACADEMIC_PROGRAMS: "Akademik Programlar Orkestratörü",
    Department.FINANCE: "Finans Orkestratörü",
}
...
def _format_agent_flow(
    routing_strategy: str | None,
    agents_involved: list[str],
    responses: list[DepartmentResponse] | None = None,
) -> str:
    response_flow = _format_agent_flow_from_responses(routing_strategy, responses or [])
    if response_flow:
    ...
    if normalized_strategy == "parallel" and len(agents) > 1:
        if agents[-1] == "Ana Orkestratör":
            return f"Ana Orkestratör -> Paralel: {' + '.join(agents[:-1])} -> Ana Orkestratör"
        return " + ".join(agents)
    return " -> ".join(agents)


def _format_agent_flow_from_responses(
    routing_strategy: str | None,
    responses: list[DepartmentResponse],
) -> str | None:
    ...
    has_multi_intent = any(
        ...
        and isinstance(response.metadata.get("multi_intent_subtask"), dict)
        for response in meaningful
    )

    branch_flows: list[str] = []
    seen: set[str] = set()
    for response in meaningful:
        branch = _format_response_branch_flow(response)
    ...
    normalized_strategy = str(routing_strategy or "").strip().lower()
    if has_multi_intent:
        planner_prefix = "Ana Orkestratör -> Alt Başlık Planlayıcı -> "
        if normalized_strategy == "parallel" and len(branch_flows) > 1:
            branches = " + ".join(f"[{branch}]" for branch in branch_flows)
            return f"{planner_prefix}Paralel: {branches} -> Ana Orkestratör"
        return f"{planner_prefix}{branch_flows[0]} -> Ana Orkestratör"
    if normalized_strategy == "parallel" and len(branch_flows) > 1:
        branches = " + ".join(f"[{branch}]" for branch in branch_flows)
        return f"Ana Orkestratör -> Paralel: {branches} -> Ana Orkestratör"
    return f"Ana Orkestratör -> {branch_flows[0]} -> Ana Orkestratör"


def _format_response_branch_flow(response: DepartmentResponse) -> str:
    department_label = _DEPARTMENT_ORCHESTRATOR_LABELS.get(
        response.department,
        f"{DEPARTMENT_SECTION_TITLES.get(response.department, response.department.value)} Orkestratörü",
    )
    agent_id = _response_selected_agent_id(response)
    agent_label = _format_agent_label(agent_id)
    department_base = department_label.removesuffix(" Orkestratörü")
    if not agent_label or normalize_text(agent_label) == normalize_text(department_base):
        return department_label
    specialist_label = _format_specialist_agent_label(agent_label)
    return f"{department_label} -> {specialist_label} -> {department_label}"


def _response_selected_agent_id(response: DepartmentResponse) -> str:
    metadata = response.metadata if isinstance(response.metadata, dict) else {}
    selection = metadata.get("specialist_selection") if isinstance(metadata, dict) else None
    selection_agent_id = (
        selection.get("selected_agent_id")
...
def _format_specialist_agent_label(label: str) -> str:
    if label.endswith(("Uzmanı", "Uzman Ajanı", "Ajanı", "Orkestratörü")):
        return label
    return f"{label} Uzman Ajanı"
