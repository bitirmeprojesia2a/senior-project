...
from src.orchestrators.department_dispatch import (
    dispatch_multi_intent_subtasks,
...
from src.orchestrators.multi_intent import (
    MultiIntentPlan,
    build_multi_intent_synthesis_prompt,
    compose_multi_intent_fallback_answer,
    has_multi_intent_responses,
...
class MainOrchestrator:
            ...
            with profile_stage("main.handle_query", context_id=context_id):
                ...
                if profiler is not None:
                    profiler.set_attribute(
                        ...
                        {
                            "context_service_enabled": self.conversation_service is not None,
                            "skipped": skip_conversation_resolution,
                            "skip_reason": "personal_data_query" if skip_conversation_resolution else None,
                            "is_follow_up": conversation_resolution.is_follow_up,
                            "used_context": conversation_resolution.used_context,
                            "active_topic": conversation_resolution.active_topic,
                            "effective_query": conversation_resolution.effective_query,
                            "rewrite_method": conversation_resolution.rewrite_method,
                            "department_hints": [
                                department.value for department in conversation_resolution.department_hints
                            ],
                            "router_preferred_departments": [
                                department.value for department in routing_department_hints
                            ],
                            "router_departments": [
                                department.value for department in routing.departments
                            ],
                            "task_type_hint": (
                                conversation_resolution.task_type_hint.value
                                if conversation_resolution.task_type_hint
                                else None
                            ),
                            "router_task_type": routing.task_type.value if routing.task_type else None,
                            "decision_reason": (
                                "context_used"
                                if conversation_resolution.used_context
                                else "direct_or_new_topic"
                            ),
                        },
                    )
                ...
                multi_intent_plan = self._maybe_plan_multi_intent(
                    query=effective_query,
                    student_department=student_department,
                )
                multi_intent_applied = False
                if multi_intent_plan is not None:
                    multi_intent_payload = multi_intent_plan.to_metadata()
                    metadata["multi_intent"] = multi_intent_payload
                    if profiler is not None:
                        profiler.set_attribute("multi_intent", multi_intent_payload)

                with profile_stage("main.dispatch_departments"):
                    if (
                        multi_intent_plan is not None
                        and settings.multi_intent_planner.should_apply
                        and multi_intent_plan.is_actionable(
                            threshold=settings.multi_intent_planner.confidence_threshold
                        )
                    ):
                        department_responses = await dispatch_multi_intent_subtasks(
                            department_orchestrators=self.department_orchestrators,
                            plan=multi_intent_plan,
                            context_id=context_id,
                            metadata=metadata,
                        )
                        multi_intent_applied = bool(department_responses)
                ...
                if multi_intent_plan is not None:
                    updated_multi_intent_payload = {
                        **multi_intent_plan.to_metadata(),
                        "applied": multi_intent_applied,
                    }
                    metadata["multi_intent"] = updated_multi_intent_payload
                    if profiler is not None:
                        profiler.set_attribute("multi_intent", updated_multi_intent_payload)
                ...
                if not responses:
                    ...
                    if (not multi_intent_applied) and should_fetch_related_announcements(effective_query):
                        announcement_scope = self._resolve_announcement_scope(
                            query=effective_query,
                            conversation_resolution=conversation_resolution,
                            student_department=student_department,
                    ...
                    supplemental_probe_query = (
                        None
                        if multi_intent_applied
                        else build_supplemental_announcement_probe_query(effective_query)
                    )
                ...
                with profile_stage("main.filter_low_confidence"):
                    filter_source_owner = (
                        None
                        if multi_intent_applied
                        else str((source_owner_payload or {}).get("primary") or "").strip() or None
                    )
                ...
                with profile_stage("main.compose_response"):
                    if multi_intent_applied:
                        answer, used_global_synthesis = await self._compose_multi_intent_final_answer(
                            query=effective_query,
                ...
                contract_validation = validate_answer_against_contract(
                    query=effective_query,
                    answer=answer,
                    contract=None if multi_intent_applied else answer_contract,
                )
                profiler = get_current_profiler()
                if profiler is not None:
                ...
                final_response = self._build_user_query_response(
                    ...
                    used_global_synthesis=used_global_synthesis,
                    routing_strategy=(
                        RoutingStrategy.PARALLEL.value
                        if multi_intent_applied
                        else routing.strategy.value if routing else None
                    ),
    ...
    @staticmethod
    def _maybe_plan_multi_intent(
        *,
        query: str,
        student_department: str | None,
    ) -> MultiIntentPlan | None:
        planner_settings = settings.multi_intent_planner
        if not planner_settings.enabled:
            return None
        return plan_multi_intent_query(
            query,
            mode=planner_settings.mode,
            student_department=student_department,
            max_subtasks=planner_settings.max_subtasks,
        )
    ...
    async def _compose_multi_intent_final_answer(
        self,
        *,
        query: str,
        responses: list[DepartmentResponse],
        ...
        if not has_multi_intent_responses(responses):
            return await self._compose_final_answer(
                query=query,
                responses=responses,
                llm_profile=llm_profile,
        ...
        prompt, meaningful = build_multi_intent_synthesis_prompt(query, responses)
        if not prompt:
            fallback = compose_multi_intent_fallback_answer(responses)
            return (fallback or self._compose_answers(responses), False)
        ...
        logger.info(
            "multi_intent_final_synthesis_start timeout_s=%s prompt_chars=%s subtasks=%s model=%s profile=%s",
        ...
        try:
            with profile_stage("main.multi_intent_final_synthesis"):
            ...
            logger.warning(
                "multi_intent_final_synthesis_failed reason=%s",
                type(exc).__name__,
            )
