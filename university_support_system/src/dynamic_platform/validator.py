...
def _validate_topology_shape(bundle: DynamicPlatformBundle, report: ValidationReport) -> None:
    ...
    for capability in sorted(agent_capabilities - source_capabilities):
        report.add_warning(
            "capability_without_enabled_source",
            f"Capability {capability} is handled by an agent but has no enabled source in the catalog.",
        )
