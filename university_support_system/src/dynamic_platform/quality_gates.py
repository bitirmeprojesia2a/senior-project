...
def run_quality_gates(
    ...
    *,
    ...
    gates.append(
        QualityGate(
            gate_id="dynamic_core_genericity_audit",
            status="passed" if genericity.ok else "failed",
            message="Dynamic platform core has no hardcoded domain-specific behavior."
            if genericity.ok
            else "Dynamic platform core contains domain-specific behavior.",
            detail=genericity.to_dict(),
        )
    )
