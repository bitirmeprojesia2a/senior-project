...
def test_extract_announcement_keywords_normalizes_turkish_cap():
    keywords = announcements_module.extract_announcement_keywords(
        "Çap ile alakalı duyuruları istiyorum"
    )

    assert "cap" in keywords
    assert "alakali" not in keywords


@pytest.mark.asyncio
async def test_fetch_relevant_announcements_does_not_return_unrelated_latest_for_cap_query(
    db_session,
    monkeypatch,
):
    unrelated = Announcement(
        title="OMTEL Otel Bafra pidesi siparislerinizi bekliyor",
        summary="Yemek siparisi duyurusu",
        display_summary="Yemek siparisi duyurusu",
        source_url="https://omu.edu.tr/omtel",
        faculty=None,
        unit_name=None,
        department=None,
        published_at=datetime.now(UTC),
    ...
    cap = Announcement(
        title="CAP Basvuru Sonuclari",
        summary="Cift anadal basvuru sonuclari aciklandi.",
        display_summary="Cift anadal basvuru sonuclari aciklandi.",
        source_url="https://omu.edu.tr/cap",
        faculty=None,
        unit_name=None,
        department="academic_programs",
    ...
    db_session.add_all([unrelated, cap])
    await db_session.flush()
    ...
    assert [row.title for row in rows] == ["CAP Basvuru Sonuclari"]
