...
def test_append_generation_summary_explains_parallel_agent_flow():
    ...
    assert (
        "- Ajan akışı: Ana Orkestratör -> Paralel: "
        "[Öğrenci İşleri Orkestratörü] + [Akademik Programlar Orkestratörü] -> Ana Orkestratör"
    ) in answer
...
def test_append_generation_summary_displays_specialist_agents_in_flow():
    ...
    assert "ana orkestrator" in normalized
    assert "ogrenci isleri orkestratoru" in normalized
    assert "akademik programlar orkestratoru" in normalized
