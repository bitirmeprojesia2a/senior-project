"""Smoke-test MainOrchestrator shadow A2A mode.

Shadow mode keeps the user-facing answer on the in-process path and sends the
same department request to the HTTP A2A service in the background.
"""
...
from scripts.a2a_main_http_smoke import (
    _FixedFinanceRouter,
    _NoopTelemetry,
    _UnusedAnnouncementAgent,
    _UnusedLLMService,
)
...
def parse_args() -> argparse.Namespace:
    parser = argparse.ArgumentParser(
        description="Start finance agent service and smoke-test MainOrchestrator shadow A2A mode."
    )
    ...
    parser.add_argument("--shadow-timeout", type=float, default=45.0)
...
async def async_main() -> None:
    ...
    log_path = Path(args.log_file) if args.log_file else Path(gettempdir()) / f"omu_a2a_shadow_finance_{args.port}.log"
    ...
    previous = _configure_parent_shadow(
        base_url=base_url,
        internal_key=args.internal_key,
    ...
    try:
        ...
        response = await orchestrator.handle_query(
            args.query,
            context_id="a2a-main-shadow-smoke",
        ...
        if "kimliginizi dogrulamam" not in response.answer:
            raise RuntimeError("Main shadow smoke beklenen in-process auth-guard cevabini alamadi.")
        if "finance" not in response.departments_involved:
            raise RuntimeError("Main shadow smoke finance departman izini alamadi.")

        await ShadowDepartmentTransport.wait_for_pending(timeout_seconds=args.shadow_timeout)
        ...
        print_json(
            {
                ...
                "shadow": {
                    "wait_completed": True,
                    "pending_tasks": len(ShadowDepartmentTransport._pending_tasks),
                },
                "started_process": {
                    "pid": process.pid,
                    "base_url": base_url,
